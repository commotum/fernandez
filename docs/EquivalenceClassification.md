...
| `EQC-009-REAL-COLUMNS-MEASUREMENT` | `realColumn0_bottomWeight`, `realColumn1_bottomWeight`, `realTopCombination_bottomWeight_of_rebit`; `goal-1/4-STATES.md:183-187` | cross | encoded pure state | every source pure state and every normalized `Rebit` top-sector coefficient | decoded computational-basis weights | none | exact decoded agreement | sum false/true added-wire sectors; coefficient parameter only, with no target-factorization or mixed-density claim | Existing pure-input decoded observation family; mixed top excluded | G2 `9C-OUTCOMES`; G3 `3-DENSITY` for mixed extension |
...
| `EQC-013-COMPLEX-COLUMNS-MEASUREMENT` | `complexColumn0_bottomWeight`, `complexColumn1_bottomWeight`, `complexTopCombination_bottomWeight_of_qubit`; `Traceability.md:46,112` | cross | encoded pure state | every source pure state and every normalized `Qubit` top-sector coefficient | decoded computational-basis weights | none | exact decoded agreement | sum false/true added-wire sectors; coefficient parameter only, with no target-factorization or mixed-density claim | Existing pure-input observation family; no reduced-matrix equality is claimed | G2 `9C-OUTCOMES`; G3 `3-DENSITY` for mixed extension |
...
| `EQC-031-C2R-STATE` | `realifyCircuit_allCoefficient_stateIntertwining` | cross | circuit evolution on encoded raw state | every raw source column and every raw coefficient pair `ℝ × ℝ` | exact target amplitude column | none | exact `AllTopStateIntertwining` | one explicit coordinate sector; coefficients are not normalized/product-top certificates | Stage 9B proof-bearing state wrapper; outcomes remain Stage 9C | G2 `9B-WRAPPERS` |
| `EQC-032-C2R-OBSERVABLE` | `realifyCircuitOutput_bottomProbability`; `Traceability.md:33,85` | cross | normalized circuit outputs | every normalized source pure input and every normalized `Rebit` top-sector coefficient | complete decoded bottom basis probabilities | none | exact decoded agreement | sum over the two target sectors; no mixed joint-density, partial-trace, or factorization claim | Existing pure-input observation certificate | G2 `9C-OUTCOMES`; G3 `3-DENSITY` |
...
| `EQC-034-Q2C-STATE` | `complexifyCircuit_allCoefficient_stateIntertwining` | cross | circuit evolution on encoded raw state | every raw source column and every raw coefficient pair `ℂ × ℂ` | exact target amplitude column | none; no right-phase quotient | exact `AllTopStateIntertwining` | one explicit coordinate sector; coefficients are not normalized/product-top certificates | Stage 9B proof-bearing state wrapper; outcomes remain Stage 9C | G2 `9B-WRAPPERS` |
| `EQC-035-Q2C-OBSERVABLE` | `complexifyCircuitOutput_bottomProbability`; `Traceability.md:35,46,112` | cross | normalized circuit outputs | every normalized quaternionic source pure input and every normalized `Qubit` top-sector coefficient | complete decoded bottom basis probabilities | none | exact decoded agreement | sum over the two target sectors; no mixed joint-density, partial-trace, or factorization claim | Existing pure-input observation certificate | G2 `9C-OUTCOMES`; G3 `3-DENSITY` |
| `EQC-036-SCHEDULED-Q2C` | `scheduledComplexifyCircuit_exactOperatorEmbedding`, `scheduledComplexifyCircuit_allCoefficient_stateIntertwining`, `scheduledComplexifyCircuit_eval_ne_of_source_ne` | cross | one supplied schedule and translated circuit | every supplied legal schedule and raw coefficient pair; concrete gap wrapper uses the existing unequal schedules | exact operator/state action and preservation of a literal operator gap | none | exact per supplied schedule | no schedule choice, quotient, independence, normalized/product-top, or outcome claim | Stage 9B proof-bearing schedule wrappers; decoded outcomes remain Stage 9C | G2 `9B-WRAPPERS`, `9C-OUTCOMES` |
| `EQC-037-Q2R-COMPOSED` | `quaternionToRealCircuit_exactOperatorEmbedding`, `quaternionToRealCircuit_allCoefficient_stateIntertwining` | cross | quaternionic circuit and composed real translation | every ordered source circuit, raw source column, and nested `(ℂ × ℂ) × (ℝ × ℝ)` coefficients | exact composed operator and amplitude action | none | exact mapped operator/state agreement | two explicit coordinate layers; inner complex then outer real; no product-state claim | Stage 9B proof-bearing composed wrappers; Equation 63 is not a second translator and outcomes remain Stage 9C | G2 `9B-WRAPPERS`, `9C-OUTCOMES` |
| `EQC-038-POSTPROCESSING-SIMULATION` | `realifyCircuitBottomDistribution_eq`, `complexifyCircuitBottomDistribution_eq`, event/pushforward theorems; `Architecture.md:276-280` | cross | source output and decoded target output distributions | every normalized source input and explicit normalized `Rebit`/`Qubit` coefficient covered by the primary simulations | full finite distribution, all finite events, deterministic pushforwards | none | exact decoded agreement | the two target sectors are decoded before comparison; no quantum partial trace or factorization claim | Existing finite observational closure; not randomized or channel equality | G2 `9C-OUTCOMES` |
...
| `EQC-036` | One supplied legal schedule has exact operator/state wrappers, and injective complexification preserves the explicit unequal-schedule operator witness. | No schedule is chosen, quotient-identified, or proved independent; decoded schedule outcomes remain Stage 9C work. |
...
identity compiler. Stage 9C still owns the five decoded outcome families.
