...
| `EQC-031-C2R-STATE` | `eval_realifyCircuit_mulVec_wireReal*`; `Architecture.md:244-251` | cross | circuit evolution on encoded state | every source column; canonical columns and every supplied `Rebit` coefficient combination where stated | exact target amplitude column | none | exact intertwining | one shared two-sector target coordinate; the coefficient is not a product-factor certificate | Existing cross-model state certificate | G2 `9B-WRAPPERS` |
...
| `EQC-034-Q2C-STATE` | `eval_complexifyCircuit_mulVec_wireComplex*`; `Architecture.md:244-259` | cross | circuit evolution on encoded state | every source column; canonical columns and every supplied `Qubit` coefficient combination where stated | exact target amplitude column | quaternion source state phase remains right-sided | exact intertwining | one shared two-sector target coordinate; the coefficient is not a product-factor certificate | Existing cross-model state certificate | G2 `9B-WRAPPERS` |
...
| `EQC-036-SCHEDULED-Q2C` | `scheduledQuaternionToComplex_exactSimulation`; `goal-1/7-ORDERING.md:169-173` | cross | one supplied scheduled source circuit and translated circuit | every supplied legal schedule; raw source columns for intertwining and normalized source plus normalized `Qubit` coefficient where probabilities are stated | operator, state, decoded basis probabilities, and resources remain separate | none | exact per schedule | no schedule choice, quotient, independence, or top/bottom factorization claim | Existing directional per-schedule certificate family | G2 `9B-WRAPPERS`, `9C-OUTCOMES` |
| `EQC-037-Q2R-COMPOSED` | `eval_quaternionToRealCircuit`, nested state theorem, `wireQuaternionToRealBottomWeight`; `Architecture.md:261-267` | cross | quaternionic circuit and composed real translation | every ordered source circuit; raw columns for intertwining and normalized source with explicit normalized coefficient parameters for outcomes | operator, nested state, four-sector bottom probabilities | none | exact mapped/decoded agreement | two shared added sector coordinates; sum four assignments, with no product-state claim | Existing compositional certificate; direct Equation 63 is not a second circuit translator | G2 `9B-WRAPPERS`, `9C-OUTCOMES` |
...
representative-coordinate results for the four canonical state columns. It
does not change the frozen cohort or finalize the circuit and outcome families
owned by Stages 9B and 9C. The stable allocation is 58 declarations: 38 generic
relations and laws plus 20 concrete encoding declarations.
