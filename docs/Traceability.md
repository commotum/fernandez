...
| `FER03-L04-REALIFICATION-INTERTWINES-STATES` | L489–558, Eq. 24–27 | `h(U) h₀(ψ)=h₀(Uψ)` and corresponding `h₁` identity. | P0 / `Matrix/Realification.lean`, 4-STATES | **partially formalized**: the correctly typed `h₀` identity is `realify_mulVec`; the second-column `h₁` identity and state-level packaging remain Stage 4 |
