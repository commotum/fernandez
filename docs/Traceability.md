...
change a paper row's terminal status. Stage 4A now adds normalized
...
`Nonempty ray ↔ Nonempty index` boundary. It does not yet descend evolution or
outcomes and does not establish the cross-model embedding boundary, so the two
transferred ray rows remain partial through this milestone. Channels remain
Stage 7.
...
|---|---|---|---|---|
| `FER03-D01-REBIT` | L92–115, Eq. 4–6 | Normalized vectors in `ℝ²`, with optional pure-state quotient by `±1`. Depends on finite real state conventions. | P2 / `State/Basic.lean`, `State/RealPhase.lean`, `State/Ray.lean`, `Semantics/StatePhase.lean` | **partially formalized**: `Rebit` is explicitly normalized and `RebitRay` is the `Bool`-indexed normalized quotient. `RealRay.mk_eq_mk_iff_eq_or_eq_neg` proves literal ray equality is exactly equality or pointwise negation; the generic quotient also has representative induction/lifting and the exact index-inhabitation boundary. Representative-level basis-distribution and unitary/circuit respectfulness already exist, but descended operations and the embedding-orbit boundary remain Stage 4B/4C obligations, so this row is not yet `closedByGoal2` |
...
|---|---|---|---|---|
| `FER03-FND-COMPLEX-STATE-RAY` | L38, L73–90 | Normalized complex vectors modulo unit right/commutative phase. | P2 / `State/ComplexPhase.lean`, `State/Ray.lean`, `Semantics/StatePhase.lean` | **partially formalized**: `ComplexRay` and `QubitRay` now give literal quotient equality, with `ComplexRay.mk_eq_mk_iff` identifying it exactly with unit right phase and with representative induction/lifting plus the exact empty-index boundary. `ComplexStatePhaseEq` still supplies the representative-level basis-distribution, matrix/circuit, and normalized-unitary respectfulness needed by Stage 4B. Descended operations and the Stage 4C embedding boundary are not yet complete, so this row is not yet `closedByGoal2` |
