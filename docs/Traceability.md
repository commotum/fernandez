...
| `FER03-D04-ORDERED-CIRCUIT-OUTPUT` | L841–849 | Product of explicitly placed gates along one legal order. Depends on noncommutative-safe placement and evaluation order. | P0 / `Circuit/Basic.lean`, 7-ORDERING | **partially formalized**: `PlacedGate`, `OrderedCircuit.eval`, and its cons/append laws give the corrected fixed-order semantics; the paper's legal-order/DAG packaging remains Stage 7 |
...
| `FER03-T04-ORDERED-QUATERNIONIC-TO-COMPLEX` | L853–855 | Translate a fixed ordered `n`-quaterbit circuit to `n+1` qubits with one arity-`d+1` image gate per source gate; prove embedded evolution and bottom measurement equality. | P0 / `Simulation/QuaternionToComplex.lean` | **corrected and proved** by `quaternionToComplex_exactSimulation`, for an explicit fixed chronological order and corrected right-module states, with separate operator/state/probability/resource theorems (C-006/C-010/C-024) |
...
| `FER03-Q-KRONECKER-INTERCHANGE` | L822–830, Eq. 46 | Prove commutative case and correct entrywise-commutation sufficient condition; disprove “only if C,D are 0–1.” | P0 / `Circuit/Placement.lean`, 7-ORDERING | **partially formalized**: `kronecker_one_mul` proves the exact noncommutative identity-complement law needed by placement; the general entrywise-commutation criterion and counterexample remain Stage 7 (C-009) |
