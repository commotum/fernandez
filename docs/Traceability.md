...
change a paper row's terminal status. Stage 4A adds normalized
...
channel, approximation, or cross-model embedding theorem. Consequently the
two transferred ray rows remain partial and not `closedByGoal2` solely pending
the Stage 4C embedding-orbit boundary. Channels remain Stage 7.

## Numbered definitions, theorems, lemmas, and corollary

| ID | Source | Concise target and dependencies | Priority / stage | Status |
|---|---|---|---|---|
| `FER03-D01-REBIT` | L92–115, Eq. 4–6 | Normalized vectors in `ℝ²`, with optional pure-state quotient by `±1`. Depends on finite real state conventions. | P2 / `State/Basic.lean`, `State/RealPhase.lean`, `State/Ray.lean`, `State/RayObservables.lean`, `State/RayEvolution.lean`, `Semantics/Ray.lean` | **partially formalized**: `Rebit` is explicitly normalized and `RebitRay` is the `Bool`-indexed normalized quotient. `RealRay.mk_eq_mk_iff_eq_or_eq_neg` proves literal ray equality is exactly equality or pointwise negation; the generic quotient also has representative induction/lifting and the exact index-inhabitation boundary. `RealRay.distribution`, `basisWeight`, `eventWeight`, and deterministic `pushforward` now descend the complete finite computational-basis outcome API, while `evolveUnitary` and locally-unitary `evolveCircuit` have exact representative, identity, matrix-composition, empty-circuit, and chronological-append laws. The cross-model embedding-orbit boundary remains the Stage 4C obligation, so this row is not yet `closedByGoal2` |
...
|---|---|---|---|---|
| `FER03-FND-COMPLEX-STATE-RAY` | L38, L73–90 | Normalized complex vectors modulo unit right/commutative phase. | P2 / `State/ComplexPhase.lean`, `State/Ray.lean`, `State/RayObservables.lean`, `State/RayEvolution.lean`, `Semantics/Ray.lean` | **partially formalized**: `ComplexRay` and `QubitRay` give literal quotient equality, with `ComplexRay.mk_eq_mk_iff` identifying it exactly with unit right phase and with representative induction/lifting plus the exact empty-index boundary. `ComplexRay.distribution`, `basisWeight`, `eventWeight`, and deterministic `pushforward` now descend representative outcomes; `evolveUnitary` and locally-unitary `evolveCircuit` descend compatible evolution with exact identity, matrix-order, empty-circuit, and append laws. `ComplexStatePhaseEq.iff_complexRay_mk_eq` is the explicit bridge between representative semantics and quotient equality. The Stage 4C embedding-orbit boundary is not yet complete, so this row remains partially formalized and is not yet `closedByGoal2` |
...
| `FER03-R-PHASE-TRACKING-INTERPRETATION` | L681–687, FN7 | Top rebit records the real/imaginary relation; heuristic phase-kickback analogy. | P2 / `State/Realification.lean`, `Simulation/NonProductWitness.lean` | **partially formalized**: the exact sector lemmas identify the top-false/top-true coordinates with real and signed imaginary components, and the non-product witness shows why the top wire need not have an independent pure value. “Recording phase” and phase-kickback remain heuristic interpretations, intentionally not theorem statements |
