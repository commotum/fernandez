...
| `FER03-L03-REAL-CIRCUIT-COMPOSITION` | L438–487, Eq. 23 | Evaluation of translated ordered circuit equals realification of source evaluation. Depends on L01 and local placement naturality. | P0 / 5-CIRCUITS, 6-SIMULATION | **partially formalized**: the missing local prerequisite is proved by `wireRealify_place` and `realifyPlacedGate_denote`; ordered-list induction remains Stage 6 |
...
| `FER03-L08-QUATERNIONIC-CIRCUIT-COMPOSITION` | L967–980 | Translated ordered operator equals `ĥ(Qσ)`. Depends on L06 and placement naturality. | P0 / 5-CIRCUITS, 6-SIMULATION | **partially formalized**: the source-omitted noncommutative local prerequisite is proved by `wireComplexify_place` and `complexifyPlacedGate_denote`; ordered-list induction remains Stage 6 |
