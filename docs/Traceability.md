...
by itself close the then-pending Stage 9B circuit/operator or Stage 9C outcome
families. In
...
Stage 9B adds 16 proof-bearing declarations in
`Semantics/SimulationWrappers.lean` without changing any paper row's terminal
status or adding a paper correction. All eleven assigned frozen families now
have exact directional wrappers at their proved operator, raw-state,
reindexing, supplied-schedule, composed, or conditional-compiler scope. Raw
coefficient pairs are not normalized/product top states; Equation 63 remains a
named row/column reindexing rather than a circuit translator; no schedule is
chosen or proved independent; and `ExactGateCompiler` remains supplied data,
not an existence, synthesis, approximation, or runtime result. Stage 9C still
owns decoded basis weights, distributions, events, and postprocessing, and no
cross-model channel or all-effect row is closed by Stage 9B.

Stage 9C adds proof-bearing outcome wrappers at all seven assigned touchpoints
without changing paper-row terminal statuses or adding a correction. Full one-
and two-added-wire target distributions remain visible until explicit decoders
are applied; one-wire decoding equals deterministic `pushforward tailBits`,
while composed decoding removes the outer realification wire and then the inner
complexification wire. Raw point weights require no local unitarity and are not
called probabilities for unnormalized sources. Distribution, finite-event, and
deterministic-pushforward wrappers use locally unitary circuits and normalized
sources; scheduled forms retain one supplied schedule. These results add no
...
| `FER03-D02-REAL-ALGORITHM` | L140–142 | Classical uniform generator, real circuit measurement, and postprocessing. Depends on circuit/output semantics and an encoding model. | P2 / `State/Distribution.lean`, `Simulation/OutcomeDecoder.lean`, `Simulation/Postprocessing.lean`, `Semantics/SimulationOutcomes.lean`, 8-RESOURCES | **partially formalized**: fixed finite real-circuit full target distributions are explicitly decoded, with all finite events and every deterministic finite postprocessing map covered; no discrete circuit encoding, uniform classical generator, randomized machine, or runtime theorem is supplied |
...
| `FER03-D05-QUATERNIONIC-ALGORITHM` | L851 | Uniform generator emits circuit plus legal ordering; ordered state is measured/postprocessed. | P2 / `Circuit/Ordering.lean`, `Simulation/Scheduled.lean`, `Semantics/SimulationWrappers.lean`, `Semantics/SimulationOutcomes.lean`, `Simulation/Postprocessing.lean`, 8-RESOURCES | **partially formalized**: the finite gate family, supplied legal schedule, ordered normalized output, exact per-schedule simulation, explicit full-target decoding, finite events, and deterministic finite postprocessing core is present. Point weights need no local unitarity; normalized levels use pointwise locally unitary gates. No schedule is chosen, and no discrete encoding, uniform classical generator, randomized postprocessor, or runtime model is claimed |
...
| `FER03-T02-COMPLEX-CIRCUIT-TO-REAL` | L167–170 | Translate `n`-qubit, `s`-gate, arity-`≤d` ordered circuits to `n+1` rebits with `s` image gates of arity `≤d+1`; separately prove operator and observable preservation. | P0 / `Simulation/ComplexToReal.lean`, `Semantics/SimulationWrappers.lean`, `Semantics/SimulationOutcomes.lean`, `Simulation/Postprocessing.lean`, `Simulation/Resources.lean` | **corrected and proved** by `complexToReal_exactSimulation`, with separate operator/state/probability/count/width/arity results; equality of every finite event and deterministic finite pushforward; and exact literal shared-top support depth and dense-slot consequences. Stage 9B classifies exact evaluator/raw-state action; Stage 9C starts from the full real target carrier and separately wraps raw point weights, normalized decoded distributions, events, and deterministic pushforwards. None is a runtime, randomized, channel, or synthesis theorem (C-010/C-019/C-024) |
...
| `FER03-T04-ORDERED-QUATERNIONIC-TO-COMPLEX` | L853–855 | Translate a fixed ordered `n`-quaterbit circuit to `n+1` qubits with one arity-`d+1` image gate per source gate; prove embedded evolution and bottom measurement equality. | P0 / `Simulation/QuaternionToComplex.lean`, `Simulation/Scheduled.lean`, `Semantics/SimulationWrappers.lean`, `Semantics/SimulationOutcomes.lean`, `Simulation/Postprocessing.lean`, `Simulation/Resources.lean` | **corrected and proved** by `quaternionToComplex_exactSimulation` for arbitrary chronological circuits and `scheduledQuaternionToComplex_exactSimulation` for each supplied legal schedule. Stage 9B adds exact primary/scheduled operator/raw-state wrappers; Stage 9C adds explicit full-target decoded distributions, events, and deterministic pushforwards for primary and supplied-schedule forms. Point weights need no local unitarity, normalized levels do, and the schedule is retained rather than chosen or identified. No runtime or randomized/channel claim is inferred (C-006/C-010/C-019/C-024) |
...
| `FER03-L03-REAL-CIRCUIT-COMPOSITION` | L438–487, Eq. 23 | Evaluation of translated ordered circuit equals realification of source evaluation. Depends on L01 and local placement naturality. | P0 / `Simulation/ComplexToReal.lean`, `Semantics/SimulationWrappers.lean` | **corrected and proved**, generalized to arbitrary finite wire types/supports by `eval_realifyCircuit`; Stage 9B packages the one-gate denotation as `realifyPlacedGate_exactOperatorEmbedding` and the full equality through `wireRealify` as `realifyCircuit_exactOperatorEmbedding`, never same-space circuit equality |
| `FER03-L04-REALIFICATION-INTERTWINES-STATES` | L489–558, Eq. 24–27 | `h(U) h₀(ψ)=h₀(Uψ)` and corresponding `h₁` identity. | P0 / `State/Realification.lean`, `Semantics/SimulationEncoding.lean`, `Semantics/SimulationWrappers.lean` | **corrected and proved** for compatible rectangular matrices by `realify_mulVec_realColumn0` and `realify_mulVec_realColumn1`; the ill-typed source derivation is replaced per C-021. Stage 9A proves both raw column maps are explicit-decoder `ℝ`-linear equivalences and both normalized representative maps are `Equiv`s. Stage 9B packages the stronger all-raw-coefficient theorem as `realifyMatrix_allCoefficient_stateIntertwining`, with both canonical pairs checked in the non-root audit |
| `FER03-L05-REAL-REDUCED-MEASUREMENT` | L561–659, Eq. 28–33 | Equal reduced real matrices for both encodings and pointwise equality of computational-basis diagonals/weights. | P0 / `State/Realification.lean`, `Simulation/OutcomeDecoder.lean`, `Semantics/SimulationOutcomes.lean` | **corrected and proved** directly: the reduced-outer and canonical bottom-weight theorems remain, while Stage 9C packages every raw column and normalized `Rebit` coefficient as `realTopCombination_allRebit_raw_decodedBasisWeightAgreement` through the explicit two-sector decoder. Raw weights need no source normalization; no generic partial trace is claimed |
...
| `FER03-L08-QUATERNIONIC-CIRCUIT-COMPOSITION` | L967–980 | Translated ordered operator equals `ĥ(Qσ)`. Depends on L06 and placement naturality. | P0 / `Simulation/QuaternionToComplex.lean`, `Semantics/SimulationWrappers.lean` | **corrected and proved** by `eval_complexifyCircuit`; it fills the source's missing noncommutative local proof before applying ordered-list induction. Stage 9B packages the one-gate denotation as `complexifyPlacedGate_exactOperatorEmbedding` and the full result through explicit `wireComplexify` as `complexifyCircuit_exactOperatorEmbedding` |
| `FER03-L09-QUATERNION-STATE-INTERTWINING` | L983–1073, Eq. 52–57 | `ĥ(Q) ĥ₀(ψ)=ĥ₀(Qψ)` and corresponding `ĥ₁` identity. | P0 / `State/Complexification.lean`, `Semantics/SimulationEncoding.lean`, `Semantics/SimulationWrappers.lean` | **corrected and proved** for compatible rectangular matrices by `complexify_mulVec_complexColumn0` and `complexify_mulVec_complexColumn1`, with the missing source output relation eliminated (C-012/C-021). Stage 9A proves both raw column maps are explicit-decoder `ℝ`-linear equivalences and both normalized representative maps are `Equiv`s. Stage 9B packages the stronger all-raw-coefficient theorem as `complexifyMatrix_allCoefficient_stateIntertwining`, with both canonical pairs checked in the non-root audit |
| `FER03-L10-QUATERNION-COMPUTATIONAL-MEASUREMENT` | L1075–1171, Eq. 58–62 | Each complex encoding preserves bottom computational-basis weights after summing over top qubit. | P0 / `State/Complexification.lean`, `Simulation/OutcomeDecoder.lean`, `Semantics/SimulationOutcomes.lean` | **proved as stated** at the observable level by the canonical bottom-weight theorems; Stage 9C packages every raw quaternionic column and normalized `Qubit` coefficient as `complexTopCombination_allQubit_raw_decodedBasisWeightAgreement` through the explicit two-sector decoder. The proof uses scalar norm decomposition rather than the paper's imprecise density algebra and implies no reduced density |
| `FER03-C01-ORDERED-QUATERNIONIC-TO-REAL` | L1202–1221, Eq. 63 | Compose T04 and T02 to simulate with `n+2` rebits; also verify the direct `4N` real representation. | P1 / `Simulation/QuaternionToReal.lean`, `Simulation/OutcomeDecoder.lean`, `Semantics/SimulationWrappers.lean`, `Semantics/SimulationOutcomes.lean`, `Matrix/QuaternionRealification.lean`, `Simulation/Resources.lean` | **corrected and proved** compositionally by the exact operator/state/four-sector theorems. Stage 9B packages operator and nested raw-coefficient action; Stage 9C packages raw decoded weights plus normalized full-target distribution, event, and deterministic-pushforward agreement, decoding the outer realification wire before the inner complexification wire. Independently, Equation 63 is only a named row/column-reindexed matrix embedding, not a direct wire-facing translator. No product/mixed-top, partial-trace, channel, randomized, or resource upgrade is claimed |
...
| `FER03-FND-FINITE-PRECISION` | FN6 | Classical descriptions contain finite-precision approximations. | P1 / `Semantics/Approximation/*.lean`; Goal 3 `8-ENCODING`, `9-APPROX` | **partially formalized** as a rigorous semantic boundary: exact abstract matrices and exact certified compilation are separated from Euclidean induced real/complex operator distance, native quaternionic operator distance, phase/sign and right-state-ray error budgets, and half-L1 total variation. Zero-budget exact bridges, additive error laws, normalized output-column bounds, event/pushforward contraction, and fixed-budget strictness witnesses are proved. No discrete scalar encoding, rounding/quantization, code-to-value certificate, accumulated gatewise circuit error, approximate compiler/synthesis construction, or finite-precision runtime is available (C-024) |
...
| `FER03-R-ARBITRARY-TOP-REBIT` | L661 | Any normalized pure or mixed product top rebit yields the same bottom basis statistics. | P1 / `State/Realification.lean`, `State/RealificationOrbit.lean`, `State/RealificationOrbitObservables.lean`, `Semantics/SimulationOutcomes.lean` | **partially formalized**: every normalized `Rebit` coefficient pair is covered at raw decoded-weight level, and Stage 4C proves every normalized coefficient encoding lies in the canonical `RealSectorOrbit`. Stage 9C's wrapper still treats the coefficient only as coordinate data; it does not assert pure/product top-bottom factorization. The non-product witness refutes that reading, and no cross-model mixed product-density encoder, no-correlation certificate, partial trace, or decoded-effect theorem closes the mixed extension |
...
| `FER03-R-SUBSET-MEASUREMENTS` | L673 | All marginals of bottom computational-basis outcomes are preserved. | P1 / `State/Distribution.lean`, `Simulation/OutcomeDecoder.lean`, `Simulation/Postprocessing.lean`, `Semantics/SimulationOutcomes.lean` | **corrected and proved** from the full real target distribution after the explicit one-wire decoder, for every finite bottom event and every deterministic map to a finite output type. The decoder equals `pushforward tailBits`; no quantum partial trace or randomized postprocessor is inferred |
| `FER03-R-COMPUTATIONAL-UNIVERSALITY` | L675–679 | Image of a quantum-universal family computationally simulates it with shared top rebit. | P1 / `Simulation/ComplexToReal.lean`, `Simulation/OutcomeDecoder.lean`, `Semantics/SimulationOutcomes.lean`, `Simulation/Postprocessing.lean` | **partially formalized**: every supplied finite circuit has an exact image with an equal decoded finite output distribution, equal finite-event weights, and equal pushforwards under every deterministic finite postprocessing map, but no encoded uniform circuit family, universal primitive library, compiler, randomized machine, or runtime theorem establishes the paper's computational-universality conclusion |
...
| `FER03-Q-ARBITRARY-TOP-QUBIT` | L1075 | Any normalized pure/mixed product top qubit preserves bottom basis statistics. | P1 / `State/Complexification.lean`, `Semantics/SimulationOutcomes.lean` | **partially formalized**: every normalized `Qubit` coefficient pair is covered for every raw source column by the explicit decoded-weight wrapper. The coefficient selects a canonical-column combination and does not certify a factorized target state. Same-space complex densities/channels exist, but no quaternionic-source density encoding, mixed product-density construction, no-correlation certificate, partial trace, or decoded-effect theorem is available |
| `FER03-Q-DIRECT-REALIFICATION` | L1208–1221, Eq. 63 | Verify direct `4×4` scalar representation and matrix/unitary consequences. | P1 / `Matrix/QuaternionRealification.lean`, `Matrix/QuaternionRealificationUnitary.lean`, `Matrix/ProperImage.lean`, `Semantics/SimulationWrappers.lean` | **corrected and proved**: `directRealify` has four real sectors per source index, all 16 Equation 63 entries, and the pure sector permutation `[3,1,0,2]` relation `directRealify_eq_reindex` to `realify (complexify A)`. Stage 9B exposes this exact named row/column reindexing as `directRealify_exactOperatorEmbedding`, not a second circuit translator. The map is injective, multiplicative, star-preserving, and sends quaternionic unitaries into `SO(4N)` with determinant one. `directRealifyUnitaryEquivImage` gives the corrected isomorphism onto its image, while `directWitness_specialOrthogonal_not_directRealify` proves the image is already proper in `SO(4)` at rank one (C-025) |
...
| `FER03-RES-GENERIC-DECOMPOSITION` | L1180–1184 | Generic target unitary elementary-gate synthesis bound. | P1 / `Circuit/Compilation.lean`, `Semantics/SimulationWrappers.lean` | **unresolved** as an existence/bound claim: the source gives no primitive library or synthesis theorem, and dense arity-`d` input already has `4^d` scalar slots. Stage 9B's `compileRealifyCircuit_exactOperatorEmbedding` and `compileComplexifyCircuit_exactOperatorEmbedding` prove exact semantics only from supplied `ExactGateCompiler` data; they establish no compiler existence, synthesis algorithm, runtime, or claimed `2^(d+1)` bound (C-013) |
...
|---|---|---|---|---|
| `FER03-INT-QUATERNION-PHASE-QUBIT` | L1200–1204 | Document component-subspace intuition; do not claim a qubit literally stores all unit-quaternion degrees of freedom. | P2 / `State/Complexification.lean` | **partially formalized**: `complexColumn0`/`complexColumn1`, their sector formulas, reconstruction, norm preservation, and normalized-`Qubit` coefficient measurement theorem give the exact two-complex-sector content. The coefficient is not a factorized target subsystem, and the claim that one top qubit “stores” a unit-quaternion phase is retained only as intuition, not an equality of state spaces or degree-of-freedom theorem |
