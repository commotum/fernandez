...
| `FER03-C01-ORDERED-QUATERNIONIC-TO-REAL` | L1202–1221, Eq. 63 | Compose T04 and T02 to simulate with `n+2` rebits; optionally prove direct `4N` real representation. | P1 / `Simulation/QuaternionToReal.lean`, `Simulation/Resources.lean`, 9-COVERAGE | **corrected and proved** compositionally by `quaternionToReal_exactSimulation`, `eval_quaternionToRealCircuit`, nested state intertwining, and four-sector probability preservation; the literal composite also has support depth exactly source gate count and dense-slot factor `16`. The optional direct `4×4` representation remains Stage 9 |
