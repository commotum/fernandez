...
|---|---|---|---|---|
| `FER03-D01-REBIT` | L92–115, Eq. 4–6 | Normalized vectors in `ℝ²`, with optional pure-state quotient by `±1`. Depends on finite real state conventions. | P2 / `State/Basic.lean` | **partially formalized**: `Rebit` is an explicitly normalized `Bool → ℝ` state and `Rebit.normalization` expands the equation; the optional quotient by sign is intentionally deferred |
...
| `FER03-D03-QUATERBIT` | L791–812, Eq. 44–45 | Normalized vectors in `ℍ²`, right unit phase, and basis weights. Depends on quaternion norm and right-module conventions. | P0 / `State/Basic.lean` | **corrected and proved**: `Quaterbit`, `Quaterbit.normalization`, `QuaternionState.rightPhase`, and the right-phase weight/evolution theorems repair C-005/C-006 |
...
| `FER03-D05-QUATERNIONIC-ALGORITHM` | L851 | Uniform generator emits circuit plus legal ordering; ordered state is measured/postprocessed. | P2 / `Circuit/Ordering.lean`, `Simulation/Scheduled.lean`, 8-RESOURCES | **partially formalized**: the finite gate-family, supplied-legal-schedule, ordered evaluation, normalized output, and exact per-schedule simulation core is present; no uniform classical generator, encoding, runtime, or postprocessing interface is claimed |
...
| `FER03-T04-ORDERED-QUATERNIONIC-TO-COMPLEX` | L853–855 | Translate a fixed ordered `n`-quaterbit circuit to `n+1` qubits with one arity-`d+1` image gate per source gate; prove embedded evolution and bottom measurement equality. | P0 / `Simulation/QuaternionToComplex.lean`, `Simulation/Scheduled.lean` | **corrected and proved** by `quaternionToComplex_exactSimulation` for arbitrary explicit chronological circuits and `scheduledQuaternionToComplex_exactSimulation` for each supplied finite legal schedule, with corrected right-module states and separate operator/state/probability/resource conclusions. The schedule is retained, not chosen or quotient-identified (C-006/C-010/C-024) |
...
| `FER03-L04-REALIFICATION-INTERTWINES-STATES` | L489–558, Eq. 24–27 | `h(U) h₀(ψ)=h₀(Uψ)` and corresponding `h₁` identity. | P0 / `State/Realification.lean` | **corrected and proved** for compatible rectangular matrices by `realify_mulVec_realColumn0` and `realify_mulVec_realColumn1`; the ill-typed source derivation is replaced per C-021 |
...
| `FER03-C01-ORDERED-QUATERNIONIC-TO-REAL` | L1202–1221, Eq. 63 | Compose T04 and T02 to simulate with `n+2` rebits; optionally prove direct `4N` real representation. | P1 / `Simulation/QuaternionToReal.lean`, 9-COVERAGE | **corrected and proved** compositionally by `quaternionToReal_exactSimulation`, `eval_quaternionToRealCircuit`, nested state intertwining, and four-sector probability preservation; the optional direct `4×4` representation remains Stage 9 |
...
| `FER03-FND-OUTCOME-EQUIVALENCE-SUFFICES` | L140–142 | Classical postprocessing only observes the output distribution. | P0 / `Simulation/ComplexToReal.lean`, `Simulation/QuaternionToComplex.lean` | **corrected and proved** at the finite circuit boundary by `realifyCircuitOutput_bottomProbability` and `complexifyCircuitOutput_bottomProbability`; external classical postprocessing is intentionally not encoded |
...
| `FER03-R-TOPO-SORT-COMPLEXITY` | L336, FN5 | Topological sorting is efficient; circuit/TM equivalence is external. | P2 / `Circuit/Ordering.lean`, 8-RESOURCES | **partially formalized**: `LegalSchedule` packages a supplied finite legal order and its certificate, but no topological-sorting algorithm, runtime proof, or circuit/TM equivalence is formalized |
...
| ID | Source | Target / dependencies | Priority / stage | Status |
|---|---|---|---|---|
| `FER03-RES-Q-WIDTH-DEPTH` | L1176–1180 | Abstract ordered simulation width `n+1`; shared top wire can serialize depth to `s`. | P1 / 6-SIMULATION, 8-RESOURCES | **partially formalized**: exact `+1` width is `width_complexifyCircuit`; depth requires the Stage 8 scheduling model |
